...
count_of_points = int_check('Введите число m - количество точек в таблице => ')
...
preparatory_table = create_preparatory_table(function, count_of_points, left_border, right_border)

print_table(preparatory_table)

while (True):
    desired_point = float_check('Введите точку, в которой хотите найти приближенное значение функции => ')
    ...
    print(f'Значение в точке {desired_point} вычисленное с помощью интерполяционного многочлена Лагранжа - {desired_value}')

    inaccuracy = get_inaccuracy(desired_value, desired_point, function)

    print(f'Фактическая погрешность значения - {inaccuracy}')

    decision = input('Хотите ввести новые значения искомой точки и степень многочлена? (Y/N)')
